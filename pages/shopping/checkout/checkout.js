...
Page({
  ...
  getCheckoutInfo: function () {
    ...
    util.request(url, { addressId: that.data.addressId, couponId: that.data.couponId, type: buyType }).then(function (res) {
      ...
      } else if (res.errno == 40002 || res.errno == 10001 || res.errno == 40003 || res.errno == 40004) {
        wx.showModal({
          title: '错误',
          content: res.errmsg,
          showCancel: false,
  ...
  submitOrder: function () {
    ...
    util.request(api.OrderSubmit, { addressId: this.data.addressId, couponId: this.data.couponId, type: this.data.buyType, inviteCode: app.globalData.contextId }, 'POST', 'application/json').then(res => {
      if (res.errno === 0) {
